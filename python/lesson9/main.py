...
def handle_message(event):
    ...
    line_bot_api.reply_message(event.reply_token, message)from flask import Flask,render_template_string, request, jsonify,abort
from google import genai
from dotenv import load_dotenv
import os
from linebot import LineBotApi, WebhookHandler
...
def chat():
    ...
    try:
        response = client.models.generate_content(
            model="gemini-2.5-flash", contents=f"{question},回應請輸出成為html格式,請記得您的名字是`Sam AI助手`"
        )
        html_format = response.text.replace("```html","").replace("```","")
        return jsonify({'html': html_format})
    except Exception as e:
